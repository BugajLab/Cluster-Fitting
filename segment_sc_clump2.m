function [clump_inds,new_center] = segment_sc_clump2(image,cell_inds,cluster_center,parameter,exclude)
...
%Make new cell inds that don't include background
im_vals = image(:);
im_med = median(im_vals);
back_vals = im_vals(im_vals<im_med);
sd3 = 3*std(back_vals);

old_cell_inds = cell_inds;
cell_vals = image(cell_inds);
out_inds = find(cell_vals < (im_med+sd3));
cell_inds(out_inds) = [];

%Making single cell image
cell_image = zeros(sz);
cell_image(cell_inds) = image(cell_inds);

%Subtract background
vals = image(cell_inds);
cell_image = cell_image-min(vals);
cell_image(cell_image<0) = 0;

%Normalize to median
cell_vals = cell_image(cell_inds);

cell_image = (cell_image/median(cell_vals)).^(1);
...
local_area=cell_image(top:bottom,left:right);
local_area = local_area(:);
local_area(local_area == 0) = [];
b_local = median(local_area);
...
rejects = exclude;
cutoff_high = 2*cell_image(cluster_center);
iterations = 1;
