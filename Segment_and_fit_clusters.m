...
%bclov_60_2_offt01xy1c1;
stem = 'bclov_60_2_off'; 
Tvec = [1,2,3,4,7,10,13,16,19,22,25,28,31];
xy = 4; 
channel = 2;
sz = [2048,2048];

if xy < 10
   xy_label = ['xy' '0' num2str(xy)];
else
    xy_label = ['xy' num2str(xy)];
end

xy_label = ['xy' num2str(xy)];

all_filenames = cell(length(Tvec),1);
stack = zeros(sz(1),sz(2),length(Tvec));
...
if t < 10
    time_label = ['t0' num2str(t)];
else
    time_label = ['t' num2str(t)];
end

all_filenames{t_ind} = [stem time_label xy_label 'c' num2str(channel) '.tif'];
stack(:,:,t_ind) = imread(all_filenames{t_ind}); 
end
...
imax = .7 * prctile(im_vals,100-sat_p);
...
tic
   center_parameter_v = 5;   %A
   cluster_parameter_v = 2.1; %B
   ...
   for i = 1:cell_number
       ...
       im_ad = sc_medfilt(im,50,cell_inds{i});
       ...
       [clusters_v_i{j},new_center] = segment_sc_clump3(im_ad,cell_inds{i},centers_v_ij,cluster_parameter_v,ex);
...
%% Batch Centers and Clusters
   ...
   for i = 1:im_end %image loop
       ...
       for j = 1:cell_n %cell loop
           ...
           %Normalized image
           im_ad = sc_medfilt(im_i,50,cell_inds_ij);
           ...
           for k = 1:cluster_number %cluster loop
               center_master_ijk = center_master_ij(k);
                    ...
                    [cluster_master_ijk,new_center] = segment_sc_clump3(im_ad,cell_inds_ij,center_master_ijk,cluster_parameter,ex);
...
i = 13; %image which will be displayed
